...
#line 741
typedef int (*__compar_fn_t)(const void *arg_0xb7bc5458, const void *arg_0xb7bc55f0);
#line 779
__extension__ 
#line 796
__extension__ 
...
#line 260
extern double erfc(double arg_0xb7b768c8) __attribute((__leaf__)) __attribute((__nothrow__)) ;
#line 336
__extension__ 
...
unsigned int (*hashfunction)(void *arg_0xb7a20840), 
int (*key_eq_fn)(void *arg_0xb7a20c88, void *arg_0xb7a20e00));
#line 103
#line 102
int 
...
static inline void RandomInitialise(int arg_0xb78ef9b8, int arg_0xb78efb10);
static double RandomUniform(void );
...
typedef sensorState SmartBraceletC$Read$val_t;
typedef TMilli SmartBraceletC$Timer10$precision_tag;
typedef TMilli SmartBraceletC$Timer60$precision_tag;
typedef TMilli SmartBraceletC$PairingTimer$precision_tag;
...
typedef sensorState /*SmartBraceletAppC.FakeSensorC.FakeSensorP*/FakeSensorP$0$Read$val_t;
...
static void SmartBraceletC$Read$readDone(error_t result, SmartBraceletC$Read$val_t val);
# 78 "/opt/tinyos-main-2.1.2/tos/interfaces/Receive.nc"
static 
#line 74
message_t * 
...
static void SmartBraceletC$Timer10$fired(void );
#line 83
static void SmartBraceletC$Timer60$fired(void );
#line 83
static void SmartBraceletC$PairingTimer$fired(void );
# 80 "/opt/tinyos-main-2.1.2/tos/interfaces/AMSend.nc"
...
uint8_t len);
# 55 "/opt/tinyos-main-2.1.2/tos/interfaces/Read.nc"
static error_t SmartBraceletC$Read$read(void );
# 64 "/opt/tinyos-main-2.1.2/tos/lib/timer/Timer.nc"
static void SmartBraceletC$Timer10$startPeriodic(uint32_t dt);
...
static void SmartBraceletC$Timer60$startOneShot(uint32_t dt);
# 59 "/opt/tinyos-main-2.1.2/tos/interfaces/PacketAcknowledgements.nc"
...
message_t * msg);
# 64 "/opt/tinyos-main-2.1.2/tos/lib/timer/Timer.nc"
static void SmartBraceletC$PairingTimer$startPeriodic(uint32_t dt);
#line 78
static void SmartBraceletC$PairingTimer$stop(void );
# 88 "/opt/tinyos-main-2.1.2/tos/interfaces/AMPacket.nc"
...
#line 74
message_t * amsg);
# 34 "SmartBraceletC.nc"
bool SmartBraceletC$busy[1000];
uint16_t SmartBraceletC$msgCount[1000];


uint8_t SmartBraceletC$sender[1000];
message_t SmartBraceletC$packet[1000];
am_addr_t SmartBraceletC$pairDevice[1000];

static const char *SmartBraceletC$key[1000][4];
...
sensorState SmartBraceletC$state[1000];
sensorState SmartBraceletC$lastPos[1000];
...
static inline void SmartBraceletC$pairingSucc(void );
static void SmartBraceletC$sendMessage(void );
...
static inline void SmartBraceletC$PairingTimer$fired(void );
#line 103
static inline void SmartBraceletC$Timer10$fired(void );
...
static inline void SmartBraceletC$Timer60$fired(void );
...
#line 262
static inline void SmartBraceletC$Read$readDone(error_t result, sensorState localState);
#line 277
static inline void SmartBraceletC$pairingSucc(void );
#line 313
static void SmartBraceletC$sendMessage(void );
...
# 73 "/opt/tinyos-main-2.1.2/tos/lib/timer/Timer.nc"
inline static void SmartBraceletC$Timer60$startOneShot(uint32_t dt){
#line 73
...
inline static void SmartBraceletC$PairingTimer$stop(void ){
#line 78
...
#line 282
  if (!SmartBraceletC$busy[sim_node()] || 1) {
      sb_msg_t *message = (sb_msg_t *)SmartBraceletC$Packet$getPayload(&SmartBraceletC$packet[sim_node()], sizeof(sb_msg_t ));


      __nesc_hton_uint8(message->type.nxdata, 2);
      __nesc_hton_uint16(message->id.nxdata, SmartBraceletC$msgCount[sim_node()]);

      strcpy(message->content, SmartBraceletC$key[sim_node()][TOS_NODE_ID]);



      while (attempts < 4) {

          if (SmartBraceletC$AMSend$send(SmartBraceletC$pairDevice[sim_node()], &SmartBraceletC$packet[sim_node()], sizeof(sb_msg_t )) == SUCCESS) {
              sim_log_debug(173U, "Radio", "Radio: sending pairing confirmation to node %hhu\n", SmartBraceletC$pairDevice[sim_node()]);
              SmartBraceletC$busy[sim_node()] = TRUE;
              attempts = 5;
              SmartBraceletC$busy[sim_node()] = TRUE;
            }
          else {
              sim_log_debug(174U, "Radio", "Error in sending confirmation packet to mote %hhu\n", SmartBraceletC$pairDevice[sim_node()]);
...
#line 162
{
  ...
  if (SmartBraceletC$AMPacket$destination(bufPtr) == AM_BROADCAST_ADDR) {
      if (strcmp(message->content, SmartBraceletC$key[sim_node()][TOS_NODE_ID]) == 0) {
          SmartBraceletC$pairDevice[sim_node()] = SmartBraceletC$AMPacket$source(bufPtr);
          sim_log_debug(161U, "Radio", "I'm mote %hhu and the key received is %s\n", TOS_NODE_ID, message->content);
          sim_log_debug(162U, "Radio", "Sending confirmation packet to the other mote that is number %hhu\n", SmartBraceletC$pairDevice[sim_node()]);

          SmartBraceletC$PairingTimer$stop();
          SmartBraceletC$phase[sim_node()] = 2;
          SmartBraceletC$pairingSucc();
        }
    }
  else {
    if (SmartBraceletC$AMPacket$destination(bufPtr) == TOS_NODE_ID && strcmp(message->content, SmartBraceletC$key[sim_node()][TOS_NODE_ID]) == 0 && SmartBraceletC$phase[sim_node()] == 1) {
        sim_log_debug(163U, "Radio", "Eureka?\n");
        SmartBraceletC$sender[sim_node()] = SmartBraceletC$AMPacket$source(bufPtr);
        SmartBraceletC$phase[sim_node()] = 2;
        SmartBraceletC$PairingTimer$stop();
        sim_log_debug(164U, "Radio", "Content is %s and my key is %s, sender was %hhu\n", message->content, SmartBraceletC$key[sim_node()][TOS_NODE_ID], SmartBraceletC$sender[sim_node()]);
        sim_log_debug(165U, "Radio", "Mote %hhu is paired with %hhu\n", TOS_NODE_ID, SmartBraceletC$sender[sim_node()]);

        if (SmartBraceletC$AMSend$send(SmartBraceletC$sender[sim_node()], &SmartBraceletC$packet[sim_node()], sizeof(sb_msg_t )) == SUCCESS) {
            sim_log_debug(166U, "Radio", "Radio: pairing complete, let's start sent to node %hhu\n", SmartBraceletC$sender[sim_node()]);
          }
      }
    else {


      if (SmartBraceletC$AMPacket$destination(bufPtr) == TOS_NODE_ID && __nesc_ntoh_uint8(message->type.nxdata) == 3) {

          SmartBraceletC$sender[sim_node()] = SmartBraceletC$AMPacket$source(bufPtr);
          SmartBraceletC$pairDevice[sim_node()] = SmartBraceletC$AMPacket$source(bufPtr);
          sim_log_debug(167U, "Radio_pack", "INFO message received\n");
          sim_log_debug(168U, "Info", "Position X: %hhu, Y: %hhu\n", __nesc_ntoh_uint16(message->X.nxdata), __nesc_ntoh_uint16(message->Y.nxdata));
          sim_log_debug(169U, "Info", "Sensor status: %s\n", message->content);
          SmartBraceletC$lastPos[sim_node()].X = __nesc_ntoh_uint16(message->X.nxdata);
          SmartBraceletC$lastPos[sim_node()].Y = __nesc_ntoh_uint16(message->Y.nxdata);
          SmartBraceletC$Timer60$startOneShot(60000);


          if (strcmp(message->content, "FALLING") == 0) {
              sim_log_debug(170U, "Info", "ALERT: FALLING!\n");
            }
        }
      }
    }
...
#line 162
# 80 "/opt/tinyos-main-2.1.2/tos/interfaces/AMSend.nc"
inline static error_t /*AMQueueP.AMQueueImplP*/AMQueueImplP$0$AMSend$send(am_id_t arg_0xb750f0a0, am_addr_t addr, message_t * msg, uint8_t len){
#line 80
  unsigned char __nesc_result;
#line 80

#line 80
  __nesc_result = TossimActiveMessageC$AMSend$send(arg_0xb750f0a0, addr, msg, len);
#line 80
...
#line 53
# 64 "/opt/tinyos-main-2.1.2/tos/lib/timer/Timer.nc"
inline static void SmartBraceletC$Timer10$startPeriodic(uint32_t dt){
#line 64
...
# 78 "/opt/tinyos-main-2.1.2/tos/interfaces/Receive.nc"
inline static message_t * TossimActiveMessageC$Snoop$receive(am_id_t arg_0xb768f178, message_t * msg, void * payload, uint8_t len){
#line 78
  nx_struct message_t *__nesc_result;
#line 78

#line 78
    __nesc_result = TossimActiveMessageC$Snoop$default$receive(arg_0xb768f178, msg, payload, len);
#line 78
...
#line 104
# 64 "/opt/tinyos-main-2.1.2/tos/lib/timer/Timer.nc"
inline static void SmartBraceletC$PairingTimer$startPeriodic(uint32_t dt){
#line 64
  /*HilTimerMilliC.VirtualizeTimerC*/VirtualizeTimerC$0$Timer$startPeriodic(2U, dt);
#line 64
}
#line 64
# 68 "SmartBraceletC.nc"
static inline void SmartBraceletC$SplitControl$startDone(error_t err)
#line 68
{
  if (err == SUCCESS) {
      ...
      SmartBraceletC$PairingTimer$startPeriodic(250);
    }
  else 
#line 74
    {
...
# 100 "/opt/tinyos-main-2.1.2/tos/interfaces/Send.nc"
inline static void /*AMQueueP.AMQueueImplP*/AMQueueImplP$0$Send$sendDone(uint8_t arg_0xb75106a8, message_t * msg, error_t error){
#line 100
  switch (arg_0xb75106a8) {
#line 100
    case 0U:
#line 100
...
#line 100
    default:
#line 100
      /*AMQueueP.AMQueueImplP*/AMQueueImplP$0$Send$default$sendDone(arg_0xb75106a8, msg, error);
#line 100
      break;
#line 100
    }
...
# 262 "SmartBraceletC.nc"
static inline void SmartBraceletC$Read$readDone(error_t result, sensorState localState)
#line 262
{
  SmartBraceletC$state[sim_node()] = localState;
  sim_log_debug(171U, "Sensors", "Sensor status: %s\n", SmartBraceletC$state[sim_node()].stateName);
  sim_log_debug(172U, "Sensors", "Position X: %hhu, Y: %hhu\n", localState.X, localState.Y);
  SmartBraceletC$sendMessage();
}
...
#line 63
  SmartBraceletC$Read$readDone(result, val);
#line 63
}
#line 63
...
#line 12
{
  ...
  int num = /*SmartBraceletAppC.FakeSensorC.FakeSensorP*/FakeSensorP$0$Random$rand16() % 10;
  int x;
  int y;
...
#line 19
  ...
  x = /*SmartBraceletAppC.FakeSensorC.FakeSensorP*/FakeSensorP$0$Random$rand16();
  y = /*SmartBraceletAppC.FakeSensorC.FakeSensorP*/FakeSensorP$0$Random$rand16();
  ...
  state.Y = y;
  state.coord[1] = y;
  /*SmartBraceletAppC.FakeSensorC.FakeSensorP*/FakeSensorP$0$Read$readDone(SUCCESS, state);
}
...
# 75 "/opt/tinyos-main-2.1.2/tos/interfaces/TaskBasic.nc"
inline static void SimSchedulerBasicP$TaskBasic$runTask(uint8_t arg_0xb784b650){
#line 75
  switch (arg_0xb784b650) {
#line 75
...
#line 75
    default:
#line 75
      SimSchedulerBasicP$TaskBasic$default$runTask(arg_0xb784b650);
#line 75
      break;
#line 75
    }
...
# 55 "/opt/tinyos-main-2.1.2/tos/interfaces/Read.nc"
inline static error_t SmartBraceletC$Read$read(void ){
#line 55
  unsigned char __nesc_result;
#line 55
...
#line 111
{
  ...
  sim_log_debug(154U, "Timer60s", "ALERT: MISSING\n");
  sim_log_debug(155U, "Timer60s", "Last known location was X: %hhu, Y: %hhu\n", SmartBraceletC$lastPos[sim_node()].X, SmartBraceletC$lastPos[sim_node()].Y);
}
...
#line 82
{
  ...
  if (!SmartBraceletC$busy[sim_node()]) {
      ...
      __nesc_hton_uint8(message->type.nxdata, 1);
      __nesc_hton_uint16(message->id.nxdata, SmartBraceletC$msgCount[sim_node()]);


      strcpy(message->content, SmartBraceletC$key[sim_node()][TOS_NODE_ID]);
      if (SmartBraceletC$AMSend$send(AM_BROADCAST_ADDR, &SmartBraceletC$packet[sim_node()], sizeof(sb_msg_t )) == SUCCESS) {
          SmartBraceletC$msgCount[sim_node()]++;
          sim_log_debug(151U, "Radio", "Radio: sending broadcast pairing packet with key=%s\n", SmartBraceletC$key[sim_node()][TOS_NODE_ID]);
...
# 83 "/opt/tinyos-main-2.1.2/tos/lib/timer/Timer.nc"
inline static void /*HilTimerMilliC.VirtualizeTimerC*/VirtualizeTimerC$0$Timer$fired(uint8_t arg_0xb73903a8){
#line 83
  switch (arg_0xb73903a8) {
#line 83
    case 0U:
#line 83
      SmartBraceletC$Timer10$fired();
#line 83
      break;
#line 83
    case 1U:
#line 83
      SmartBraceletC$Timer60$fired();
#line 83
      break;
#line 83
    case 2U:
#line 83
      SmartBraceletC$PairingTimer$fired();
#line 83
      break;
#line 83
    default:
#line 83
      /*HilTimerMilliC.VirtualizeTimerC*/VirtualizeTimerC$0$Timer$default$fired(arg_0xb73903a8);
#line 83
      break;
#line 83
    }
...
#line 119
{
  if (&SmartBraceletC$packet[sim_node()] == bufPtr && error == SUCCESS) {
      sim_log_debug(156U, "Radio_sent", "Packet sent, phase = %hu\n", SmartBraceletC$phase[sim_node()]);
      SmartBraceletC$busy[sim_node()] = FALSE;

      if (SmartBraceletC$phase[sim_node()] == 2) {


          SmartBraceletC$phase[sim_node()] = 3;



          if (TOS_NODE_ID % 2 == 0) {

              sim_log_debug(157U, "OperationalMode", "------>Parent bracelet<------\n");
              SmartBraceletC$Timer60$startOneShot(60000);
            }
          else 
#line 135
            {
              ...
              sim_log_debug(158U, "OperationalMode", "------>Child bracelet<------\n");
              SmartBraceletC$Timer10$startPeriodic(10000);
            }
        }
      else {


        if (SmartBraceletC$phase[sim_node()] == 3 && SmartBraceletC$PacketAcknowledgements$wasAcked(bufPtr)) {
            sim_log_debug(159U, "Radio_ack", "INFO ack received at time %s\n", sim_time_string());
          }
        else {
#line 147
          if (SmartBraceletC$phase[sim_node()] == 3) {

              sim_log_debug(160U, "Radio_ack", "INFO ack not received at time %s\n", sim_time_string());
              SmartBraceletC$sendMessage();
            }
          }
        }
...
#line 313
{
  ...
  if (!SmartBraceletC$busy[sim_node()]) {
      ...
      __nesc_hton_uint8(message->type.nxdata, 3);
      __nesc_hton_uint16(message->id.nxdata, SmartBraceletC$msgCount[sim_node()]);
      __nesc_hton_uint16(message->X.nxdata, SmartBraceletC$state[sim_node()].X);
      __nesc_hton_uint16(message->Y.nxdata, SmartBraceletC$state[sim_node()].Y);
      strcpy(message->content, SmartBraceletC$state[sim_node()].stateName);


      SmartBraceletC$PacketAcknowledgements$requestAck(&SmartBraceletC$packet[sim_node()]);

      if (SmartBraceletC$AMSend$send(SmartBraceletC$sender[sim_node()], &SmartBraceletC$packet[sim_node()], sizeof(sb_msg_t )) == SUCCESS) {
          sim_log_debug(175U, "Radio", "Radio: sending INFO packet to node %hhu\n", SmartBraceletC$sender[sim_node()]);
                               ...
                               uintptr_t* addr, size_t* size)
{
  ...
  if (!strcmp(varname, "SmartBraceletC$msgCount"))
  {
    *addr = (uintptr_t)&SmartBraceletC$msgCount[__nesc_mote];
    *size = sizeof(SmartBraceletC$msgCount[__nesc_mote]);
    return 0;
  }
  if (!strcmp(varname, "SmartBraceletC$sender"))
  {
    *addr = (uintptr_t)&SmartBraceletC$sender[__nesc_mote];
    *size = sizeof(SmartBraceletC$sender[__nesc_mote]);
    return 0;
  }
  ...
  if (!strcmp(varname, "SmartBraceletC$pairDevice"))
  {
    *addr = (uintptr_t)&SmartBraceletC$pairDevice[__nesc_mote];
    *size = sizeof(SmartBraceletC$pairDevice[__nesc_mote]);
    return 0;
  }
  ...
  if (!strcmp(varname, "SmartBraceletC$state"))
  {
    *addr = (uintptr_t)&SmartBraceletC$state[__nesc_mote];
    *size = sizeof(SmartBraceletC$state[__nesc_mote]);
    return 0;
  }
  if (!strcmp(varname, "SmartBraceletC$lastPos"))
  {
    *addr = (uintptr_t)&SmartBraceletC$lastPos[__nesc_mote];
    *size = sizeof(SmartBraceletC$lastPos[__nesc_mote]);
    return 0;
  }
...
static void __nesc_nido_initialise(int __nesc_mote)
{
  ...
  SmartBraceletC$msgCount[__nesc_mote] = 1;
  memset((void *)&SmartBraceletC$sender[__nesc_mote], 0, sizeof SmartBraceletC$sender[__nesc_mote]);
  memset((void *)&SmartBraceletC$packet[__nesc_mote], 0, sizeof SmartBraceletC$packet[__nesc_mote]);
  memset((void *)&SmartBraceletC$pairDevice[__nesc_mote], 0, sizeof SmartBraceletC$pairDevice[__nesc_mote]);
  memcpy((void *)&SmartBraceletC$key[__nesc_mote], (void *)&(char const *[4]){ "wcru2p8ylwr6nbv584re", "wcru2p8ylwr6nbv584re", "fhhnswlfojnfna971m14", "fhhnswlfojnfna971m14" }, sizeof SmartBraceletC$key[__nesc_mote]);
  SmartBraceletC$phase[__nesc_mote] = 1;
  memset((void *)&SmartBraceletC$state[__nesc_mote], 0, sizeof SmartBraceletC$state[__nesc_mote]);
  memset((void *)&SmartBraceletC$lastPos[__nesc_mote], 0, sizeof SmartBraceletC$lastPos[__nesc_mote]);
